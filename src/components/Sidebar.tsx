...
export default function Sidebar({ activeSection, onSectionChange }: SidebarProps) {
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 flex flex-col z-10"
      style={{ background: "#020a18", borderRight: "1px solid #0f3060" }}>

      {/* ── ヘッダー（青海波パターン） ── */}
      <div className="relative overflow-hidden px-5 py-6 border-b"
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => {
                if (!isActive) {
                  (e.currentTarget as HTMLButtonElement).style.background = "rgba(15,48,96,0.6)";
                }
              }}
              onMouseLeave={(e) => {
                if (!isActive) {
            ...
            >
              {/* 漢字アイコン */}
              <div
                className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-black flex-shrink-0 transition-all duration-200"
                style={
                  isActive
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] truncate" style={{ color: isActive ? "#d4a017" : "#3a5a7a" }}>
                  {item.sub}
                </div>
              </div>
            </button>
      ...
      {/* ── フッター ── */}
      <div className="px-5 py-3 text-center text-[10px]" style={{ color: "#2a4a6a" }}>
        © 2026 霧島ファンサイト（非公式）
        <br />
        <span style={{ color: "#1a3a5a" }}>Data: Mock / 情報はフィクション</span>
      </div>
    </aside>
  );
}
